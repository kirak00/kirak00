...
function tabFunction() {
  class Tabs {
    ...
    setSelectedTab(currentTab) {
      for (var i = 0; i < this.tabs.length; i += 1) {
        var tab = this.tabs[i];
        var pageTitle = document.querySelectorAll('[data-tabTitle]') ? document.querySelectorAll('[data-tabTitle]') : '';
        pageTitle[i].classList.add('isHidden');
        if (currentTab === tab) {
          tab.setAttribute('aria-selected', 'true');
          tab.removeAttribute('tabindex');
          this.tabpanels[i].classList.add('isShow');
          this.tabs[i].parentNode.classList.add('on');
          pageTitle[i].classList.remove('isHidden');
        } else {
          tab.setAttribute('aria-selected', 'false');
          tab.tabIndex = -1;
          this.tabpanels[i].classList.remove('isShow');
          this.tabs[i].parentNode.classList.remove('on');
          // 
        }
  ...
  window.addEventListener('load', function () {
    var tablists = document.querySelectorAll('[role=tablist].tabFunc');
